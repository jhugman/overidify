...
function resolveFile(srcFile, relativeFilePath, options, config) {
  ...
  var targetFile = aliasCache(fullPath, function () {
    var dirListing = functions.getDirListing(targetDirectory, fileExt);
...
function findBestFile(stem, dirListing, options, config) {
  ...
  if (multipleValues.length === 0) {
    ...
    var onDiskModifiers = getAllModifiersFromObjectSelector(undefined, filePaths);
    var ofInterestModifiers = [],
        propertyNames = [];

    _.each(config, function (i, key) {
      var selector = config[key],
          modifiers = getAllModifiersFromSelector(key, selector),
...
function getAllModifiersFromSelector(propertyName, selector) {
  if (_.isArray(selector)) {
    return getAllModifiersFromArraySelector(propertyName, selector);
  } else if (_.isObject(selector)) {
    return getAllModifiersFromObjectSelector(propertyName, selector);
  }
}
